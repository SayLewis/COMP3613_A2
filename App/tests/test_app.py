...
class CourseIntegrationTests(unittest.TestCase):
    ...
    def test_update_course(self):
        ...
        update_course(course.id, "courseName", "Advanced OS")
...
class AllocationIntegrationTests(unittest.TestCase):
    ...
    def test_delete_allocate(self):
        ...
        create_allocation(course.id, 2)

        # Fetch allocations for the staff to get the allocation ID
        allocations = get_allocates_by_staff(2)
        self.assertGreater(len(allocations), 0, "No allocations found when one was expected")

        allocation_id = allocations[0].id  # Get the ID of the allocation
        ...
        # Check if any allocation still exists
        self.assertEqual(len(allocations_after_delete), 0, "Allocation still exists after deletion")